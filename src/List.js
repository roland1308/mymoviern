...
class List extends Component {
  ...
  async getList() {
    if (this.props.general.language === '') {
      return;
    }
    ...
    axios
      .all([request, request1, request2, request3])
      .then(
        axios.spread((...responses) => {
          ...
          if (response1.status === 200) {
            ...
            this.setState({
              error: true,
              errorMsg: response1.data.status_message,
              isListLoading: false,
            });
            return;
          }
          if (response2.status === 200) {
            ...
            this.setState({
              error: true,
              errorMsg: response2.data.status_message,
              isListLoading: false,
            });
            return;
          }
          if (response3.status === 200) {
            ...
            this.setState({
              error: true,
              errorMsg: response3.data.status_message,
              isListLoading: false,
            });
            return;
          }
  ...
  render() {
    ...
    return (
      <Layout style={styles.container}>
        <Modal
          visible={isLoading || isListLoading}
        ...
        {!isLoginVisible && !isRegisterVisible && !isLogged && (
          <Layout style={styles.containerIntro}>
            <Text category='h1'>Welcome guest!</Text>
            <Text category='s1'>To enjoy full functionality, please:</Text>
            <Layout style={[styles.buttons, {marginTop: 5, marginBottom: 0}]}>
              <Button
                status='success'
                onPress={() => this.setisLoginVisible(true)}
              >
                Log In
              </Button>
              ...
              <Button
                status='success'
                onPress={() => this.setisRegisterVisible(true)}
              >
                Register
              </Button>
            </Layout>
        ...
        {isLoginVisible && (
          <Layout style={styles.containerIntro}>
            <Layout style={{width: '80%'}}>
              <Text category='h1'>User Log In</Text>
              <Input
                value={userName}
                label='Username'
              ...
              <Layout style={styles.buttons}>
                <Button status='success' onPress={this.logUser}>
                  Log In
                </Button>
                <Button onPress={() => this.setisLoginVisible(false)}>
                  Cancel
                </Button>
              </Layout>
            </Layout>
        ...
        {isRegisterVisible && (
          <Layout style={styles.containerIntro}>
            <Layout style={{width: '80%'}}>
              <Text category='h1'>Register new user</Text>
              <Input
                value={userName}
                label='Username'
              ...
              <Input
                ...
                onChangeText={(nextPassword) =>
                  this.setChkPassword(nextPassword)
                }
              />
              ...
              <Layout style={styles.buttons}>
                <Button status='success' onPress={this.registerUser}>
                  Register
                </Button>
                <Button onPress={() => this.setisRegisterVisible(false)}>
                  Cancel
                </Button>
              </Layout>
            </Layout>
